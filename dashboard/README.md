...
| Trade Journal *(v0.6.0)* | `/journal`, `/journal/stats`, `/journal/{signal_id}`, `/journal/{signal_id}/annotate`, `/journal/export` | every paper trade in full detail: entry/SL/TP/exit, realized R + planned RR, engine votes at signal time, regime, confluence score, duration; cumulative-R equity curve with streaks and per-symbol/regime/direction breakdowns; filterable + paginated + CSV export. Every derived figure is recomputed server-side from row prices (`storage/journal.py`) — the stored `pnl_*` columns are never trusted (legacy pip-size corruption). The one write surface is operator notes/tags — annotation only, audit-logged, never feeds a gate or measurement |
