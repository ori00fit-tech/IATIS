...
| **H019 — crypto positioning as an internal confluence modulator** (funding-rate crowding/squeeze proxy + Fear&Greed as a regime-scaling amplifier — causal, penalty-only, never a standalone signal) | controlled A/B (H017 method), chronological TEST slice, BTCUSD + ETHUSD only (n=2, stricter zero-losing-symbol rule since n=2 can't tolerate the usual "≤1 losing symbol" allowance) | **Failed.** BTCUSD PF 1.246 → 1.115 (ΔPF −0.131, regressed); ETHUSD PF 1.141 → 1.199 (ΔPF +0.058). Mean ΔPF −0.037, well below the required ≥0.05, and BTCUSD alone regressing disqualifies it under H019's own zero-losing-symbol rule — a real, differentiated effect (dPF nonzero and opposite-signed between the two symbols), just not a net-beneficial or consistent one. `engines.crypto_positioning_modulator` stays FALSE (already the shipped default — no config change needed). |
