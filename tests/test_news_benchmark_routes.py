"""
tests/test_news_benchmark_routes.py
----------------------------------------
Provider Benchmark & Data Quality Lab Phase 2 — contract tests for
execution/routes/news_benchmark.py, matching tests/
test_provider_benchmark_routes.py's established fixtures/conventions.
"""
...
def _wait_for_terminal(client, run_id: str, timeout: float = 5.0) -> dict:
    ...
    while time.monotonic() < deadline:
        body = client.get(f"/research/news-benchmark/{run_id}", headers=HDR).json()
...
def test_create_requires_auth(client):
    assert client.post("/research/news-benchmark", json=_VALID_BODY).status_code == 401


def test_list_requires_auth(client):
    assert client.get("/research/news-benchmark").status_code == 401


def test_status_requires_auth(client):
    assert client.get("/research/news-benchmark/does-not-exist").status_code == 401


def test_results_requires_auth(client):
    assert client.get("/research/news-benchmark/does-not-exist/results").status_code == 401


def test_cancel_requires_auth(client):
    assert client.post("/research/news-benchmark/does-not-exist/cancel").status_code == 401


# ── Validation ──────────────────────────────────────────────────────

def test_create_rejects_unknown_profile(client):
    r = client.post("/research/news-benchmark", json={**_VALID_BODY, "profile": "not_a_profile"}, headers=HDR)
    assert r.status_code == 400
    assert "profile" in r.json()["detail"]


def test_create_rejects_empty_symbols_list(client):
    r = client.post("/research/news-benchmark", json={**_VALID_BODY, "symbols": []}, headers=HDR)
    assert r.status_code == 400


def test_create_rejects_empty_providers_list(client):
    r = client.post("/research/news-benchmark", json={**_VALID_BODY, "providers": []}, headers=HDR)
    assert r.status_code == 400


def test_create_rejects_unknown_provider(client):
    r = client.post("/research/news-benchmark", json={**_VALID_BODY, "providers": ["not_a_provider"]}, headers=HDR)
    assert r.status_code == 400
    assert "Unknown provider" in r.json()["detail"]


def test_create_rejects_hours_back_out_of_bounds(client):
    r = client.post("/research/news-benchmark", json={**_VALID_BODY, "hours_back": 0}, headers=HDR)
    assert r.status_code == 400
    r2 = client.post("/research/news-benchmark", json={**_VALID_BODY, "hours_back": 99_999}, headers=HDR)
    assert r2.status_code == 400


def test_create_rejects_limit_out_of_bounds(client):
    r = client.post("/research/news-benchmark", json={**_VALID_BODY, "limit": 0}, headers=HDR)
    assert r.status_code == 400
    r2 = client.post("/research/news-benchmark", json={**_VALID_BODY, "limit": 99_999}, headers=HDR)
    assert r2.status_code == 400
...
def test_create_allows_null_symbols_and_providers(client, monkeypatch):
    ...
    union of both providers' own mapped symbols and both providers
    (see backtest/news_benchmark.py's run_benchmark)."""
    monkeypatch.setattr("subprocess.Popen", _FakeProc)
    r = client.post("/research/news-benchmark", json={"profile": "smoke"}, headers=HDR)
...
def test_create_builds_expected_argv_and_returns_run_id(client, monkeypatch):
    ...
    r = client.post("/research/news-benchmark", json=_VALID_BODY, headers=HDR)
    assert r.status_code == 200, r.text
    body = r.json()
    ...
    assert "backtest.news_benchmark" in argv
...
def test_create_includes_providers_hours_back_and_limit_when_given(client, monkeypatch):
    ...
    r = client.post("/research/news-benchmark", json=body, headers=HDR)
    run_id = r.json()["run_id"]
    _wait_for_terminal(client, run_id)
    argv = _FakeProc.captured_argv
...
def test_list_shows_created_run(client, monkeypatch):
    ...
    listing = client.get("/research/news-benchmark", headers=HDR)
...
def test_status_404_when_unknown(client):
    r = client.get("/research/news-benchmark/does-not-exist-at-all", headers=HDR)
...
def test_results_empty_before_any_recorded(client, monkeypatch):
    ...
    run_id = r.json()["run_id"]
    _wait_for_terminal(client, run_id)
    results = client.get(f"/research/news-benchmark/{run_id}/results", headers=HDR)
    assert results.status_code == 200
    assert results.json()["results"] == []


def test_cancel_404_when_unknown(client):
    r = client.post("/research/news-benchmark/does-not-exist/cancel", headers=HDR)
    assert r.status_code == 404
...
def test_cancel_running_job(client, monkeypatch):
    ...
    while time.monotonic() < deadline:
        status = client.get(f"/research/news-benchmark/{run_id}", headers=HDR).json()
        if status.get("job_status") == "running":
            break
        time.sleep(0.02)

    cancel = client.post(f"/research/news-benchmark/{run_id}/cancel", headers=HDR)
